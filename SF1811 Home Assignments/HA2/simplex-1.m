...
while Loop == true
    ...
    if rv(1:end) >= 0
        ...
        solution(Beta) = fix(b_bar)
        disp('The optimal solution is');
        profit = -solution*nytt_c;
        ...
        A_bar_nonpositive = find(A_bar <= 0);

        is_it_empty = isempty(A_bar_nonpositive);

        comparison = zeros(length(A_bar), 1);
        
        %To make sure that the comparison works and we retrieve the correct
        %value
        if is_it_empty == true
            positive_index = find(A_bar(1:end)) ;           
        
        else
            positive_index = find(A_bar > 0);
        end
           
        
        comparison(positive_index) = b_bar(positive_index)./A_bar(positive_index);

        %To find the element in Beta that we are going to take out and
        %replace

        min_value = min(comparison(positive_index));
        no_solution = length(find(comparison <= 0));
        
        %If all elements in A_bar are <= 0, then there is no solution to
        %the problem, and we thus break the program, and print error msg0
        if no_solution == length(comparison)
